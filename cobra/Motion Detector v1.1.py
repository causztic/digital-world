...
class MotionDetector:
    def __init__(self,ceil=25):
        self.ceil = ceil
        self.capture = cv.CaptureFromCAM(0)
        cv.NamedWindow("Motion Dectector", 1)
        ...
        while True:
            ...
            while True:
                ...
                if avg > self.ceil:
                    ...
                    if now - LastOn > Idle:
                        if Display:
                            subprocess.call("monitor_off.sh",shell=False)
                            Display = False
                time.sleep(1)                
...
#            cv.DrawContours (color_image, backcontours,  _red, _green, levels, 2, cv.CV_FILLED)
            ...
            cv.ShowImage("Motion Detector", color_image)

            # wait for ESC or ENTER key
